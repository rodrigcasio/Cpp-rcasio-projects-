...
class Player{
    public:
        ...
        ~Player(){ std::cout << "Player destroyed" << std::endl << std::endl; }
};

class Sword{ 
    public:
        std::weak_ptr<Player> player_ref;
        ~Sword(){ std::cout << "Sword destroyed" << std::endl << std::endl; }
        
        void check_owner(){
            if(auto player = player_ref.lock()){           // also works "std::shared_ptr<Player> player" 
...
int main(){
    ...
    player->sword_ptr = sword;           // "sword_ptr" (from Player class) becomes the second owner of the Sword object (reference count = 2, "sword" and "sword_ptr")
    sword->player_ref = player;         // "player_ptr" (from Sword class) weakly points to Player, so Player's object reference count stays 1.

    std::cout << "sword's reference count: "<< sword.use_count() << std::endl;
    std::cout << "player's reference count: " << player.use_count() << std::endl;
    
    sword->check_owner();

    player.reset();             // drops "Player" object reference count to 0. it basically eliminating the shared_ptr called "player" which also drops automatically the weak_ptr player_ref 
